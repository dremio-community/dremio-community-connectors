#!/usr/bin/env bash
# =============================================================================
# Build script for Dremio Splunk Connector Kubernetes images
#
# Builds two Docker images:
#   1. Full custom Dremio image with Splunk plugin baked in (recommended)
...
if [[ -n "$REGISTRY" ]]; then
  FULL_IMAGE="${REGISTRY}/dremio-with-splunk:${DREMIO_TAG}"
  INIT_IMAGE="${REGISTRY}/dremio-splunk-plugin-init:${PLUGIN_VERSION}"
else
  FULL_IMAGE="dremio-with-splunk:${DREMIO_TAG}"
  INIT_IMAGE="dremio-splunk-plugin-init:${PLUGIN_VERSION}"
fi

echo "=== Dremio Splunk Connector — Kubernetes Image Builder ==="
...
if [[ "$BUILD_INIT" == true ]]; then
  ...
  if ! ls "${SCRIPT_DIR}/target/dremio-splunk-connector-"*"-plugin.jar" &>/dev/null; then
    echo ""
