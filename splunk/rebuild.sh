#!/usr/bin/env bash
# ============================================================================
# Dremio Splunk Connector — Version-aware Rebuild Script
#
# PURPOSE
...
echo -e "${BOLD}${CYAN}   Dremio Splunk Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" mkdir -p /tmp/splunk-rebuild
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/splunk-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/splunk-rebuild/"
  docker exec -u root "$TARGET" bash -c "chmod -R 777 /tmp/splunk-rebuild"

  if ! docker exec "$TARGET" bash -c "
    cd /tmp/splunk-rebuild
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/splunk-rebuild && mkdir -p /tmp/splunk-rebuild"
  # Copy source into the pod
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/splunk-rebuild -xf -
  # Build
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/splunk-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  if ! $BUILD_FAILED; then
    ...
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/splunk-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c \
    "cp /tmp/splunk-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/ && \
     chown dremio:dremio /opt/dremio/jars/3rdparty/${PLUGIN_JAR_NAME}"
  mkdir -p "${SCRIPT_DIR}/jars"
  docker cp "${TARGET}:/tmp/splunk-rebuild/target/${PLUGIN_JAR_NAME}" \
  ...
  mkdir -p "${SCRIPT_DIR}/jars"
  cp "${BUILD_TMPDIR}/target/${PLUGIN_JAR_NAME}" "${SCRIPT_DIR}/jars/"
  rm -rf "$BUILD_TMPDIR"
...
if [[ "$MODE" == "docker" ]]; then
  ...
  for i in $(seq 1 60); do
    ...
    if [[ "$HTTP" == "200" || "$HTTP" == "404" || "$HTTP" == "401" ]]; then
  ...
  warn "Bare-metal install: restart Dremio manually to load the new plugin."
