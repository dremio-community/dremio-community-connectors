# Dremio Apache Hudi Connector — Kubernetes Install Guide

This guide covers deploying the Hudi connector to **Dremio Enterprise** running on
...
This produces and pushes: `your-registry.example.com/dremio-with-hudi:26.0.5`
...
kubectl exec -n dremio deployment/dremio-coordinator -- \
  ls /opt/dremio/jars/3rdparty/dremio-hudi-connector-*.jar

# Check Dremio logs for classpath scanning
kubectl logs -n dremio deployment/dremio-coordinator \
  | grep -i "hudi\|sabot-module"
```

Open the Dremio UI → Sources → Add Source → look for **Apache Hudi**.
...
2. `hudi-plugin` — copies the Hudi plugin JAR into the same volume
...
This produces and pushes: `your-registry.example.com/dremio-hudi-plugin-init:1.0.0`

### Step 2: Configure Helm values

Edit `k8s/helm-values-init-container.yaml`:

- Replace `your-registry.example.com/dremio-hudi-plugin-init:1.0.0` with your actual image
...
kubectl logs -n dremio dremio-coordinator-0 -c hudi-plugin
...
- Confirm the JAR is present: `kubectl exec -n dremio <pod> -- ls /opt/dremio/jars/3rdparty/ | grep hudi`
- Check Dremio server logs: `kubectl logs -n dremio deployment/dremio-coordinator | grep -i hudi`
...
- Verify the image was successfully pushed: `docker manifest inspect your-registry.../dremio-hudi-plugin-init:1.0.0`

**`ClassNotFoundException` for Hudi classes at runtime**
