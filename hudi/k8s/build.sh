#!/usr/bin/env bash
# =============================================================================
# Build script for Dremio Hudi Connector Kubernetes images
#
# Builds two Docker images:
#   1. Full custom Dremio image with Hudi plugin baked in (recommended)
...
if [[ -n "$REGISTRY" ]]; then
  FULL_IMAGE="${REGISTRY}/dremio-with-hudi:${DREMIO_TAG}"
  INIT_IMAGE="${REGISTRY}/dremio-hudi-plugin-init:${PLUGIN_VERSION}"
else
  FULL_IMAGE="dremio-with-hudi:${DREMIO_TAG}"
  INIT_IMAGE="dremio-hudi-plugin-init:${PLUGIN_VERSION}"
fi

echo "=== Dremio Hudi Connector — Kubernetes Image Builder ==="
...
if [[ "$BUILD_INIT" == true ]]; then
  ...
  # Check that the JAR exists (must run mvn package first)
  if ! ls "${SCRIPT_DIR}/target/dremio-hudi-connector-"*"-plugin.jar" &>/dev/null; then
    echo ""
    ...
    echo ""
    # Attempt build; will fail gracefully if deps aren't installed
