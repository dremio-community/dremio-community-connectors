#!/bin/bash
# ============================================================================
# Dremio Apache Hudi Connector — Installer
#
...
banner() {
  ...
  echo -e "${BOLD}${CYAN}   Dremio Apache Hudi Connector Installer${RESET}"
  echo -e "${BOLD}${CYAN}════════════════════════════════════════════════${RESET}"
  echo ""
}
...
'
  ...
  if [[ "$exec_cmd" == docker* ]]; then
    ...
    docker exec -u root "${container_name}" bash -c "rm -rf /tmp/hudi-build && mkdir -p /tmp/hudi-build"
    docker cp "${SCRIPT_DIR}/." "${container_name}:/tmp/hudi-build/"
    docker exec -u root "${container_name}" bash -c "rm -rf /tmp/hudi-build/target && chown -R dremio:dremio /tmp/hudi-build"
    docker exec "${container_name}" bash -c "
      cd /tmp/hudi-build && mvn package -DskipTests -q
      echo 'Build complete.'
      ls -lh target/${PLUGIN_JAR_NAME}
    "
    # Pull the freshly-built JAR back to host jars/ for next time
    docker cp "${container_name}:/tmp/hudi-build/target/${PLUGIN_JAR_NAME}" \
      "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}" 2>/dev/null && \
      ok "Built JAR saved to jars/ for future --prebuilt installs"
    DEPLOY_FROM="/tmp/hudi-build/target/${PLUGIN_JAR_NAME}"
  else
    # kubectl: use tar pipe
    tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
      kubectl exec -i "$TARGET" -- tar -C /tmp/hudi-build -xf -
    $exec_cmd bash -c "cd /tmp/hudi-build && mvn package -DskipTests -q"
    DEPLOY_FROM="/tmp/hudi-build/target/${PLUGIN_JAR_NAME}"
  fi
  ok "Build successful"
}
...
'
    ...
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "rm -rf /tmp/hudi-build && mkdir -p /tmp/hudi-build"
    tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
      kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/hudi-build -xf -
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "cd /tmp/hudi-build && mvn package -DskipTests -q --batch-mode"
    ok "Build successful"

    step "3d" "Extracting built JAR to local jars/..."
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/hudi-build/target/${PLUGIN_JAR_NAME}" \
...
echo "  2. Go to Sources → Add Source → Apache Hudi"
echo "  3. Set the root path to your Hudi tables directory"
echo "  4. Configure the record key field (default: id)"
echo "  5. Click Save — your Hudi tables will be auto-discovered"
echo ""
echo -e "${BOLD}Example queries:${RESET}"
echo "  -- Read from a Hudi table"
echo "  SELECT * FROM hudi_source.my_table LIMIT 10;"
echo ""
echo "  -- Write a new Hudi table via CTAS"
echo "  CREATE TABLE hudi_source.output.my_table"
echo "  STORE AS (type => 'hudi')"
echo "  AS SELECT * FROM other_source.my_data;"
echo ""
echo -e "${BOLD}Note:${RESET} The record key column must be present in any CTAS SELECT list."
echo "  Configure the key field in the source settings (default: id)."
echo ""
