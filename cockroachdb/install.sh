#!/bin/bash
# ============================================================================
# Dremio CockroachDB Connector — Installer
#
...
banner() {
  ...
  echo -e "${BOLD}${CYAN}   Dremio CockroachDB Connector Installer${RESET}"
  echo -e "${BOLD}${CYAN}════════════════════════════════════════════════${RESET}"
  echo ""
}
...
if [[ "$MODE" == "docker" ]]; then
  ...
  if [[ "$JAR_MODE" == "prebuilt" ]]; then
    ...
    docker exec -u root "$TARGET" bash -c "rm -rf /tmp/cockroachdb-build && mkdir -p /tmp/cockroachdb-build"
    docker cp "${SCRIPT_DIR}/." "${TARGET}:/tmp/cockroachdb-build/"
    docker exec -u root "$TARGET" bash -c "rm -rf /tmp/cockroachdb-build/target && chown -R dremio:dremio /tmp/cockroachdb-build"
    docker exec "$TARGET" bash -c "cd /tmp/cockroachdb-build && mvn package -DskipTests -q --batch-mode"
    ok "Build successful"
    docker cp "${TARGET}:/tmp/cockroachdb-build/target/${PLUGIN_JAR_NAME}" \
      "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}" 2>/dev/null && \
      ok "Built JAR saved to jars/ for future --prebuilt installs"
    DEPLOY_FROM="/tmp/cockroachdb-build/target/${PLUGIN_JAR_NAME}"
  fi
  ...
  if [[ "$JAR_MODE" == "prebuilt" ]]; then
    ...
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "rm -rf /tmp/cockroachdb-build && mkdir -p /tmp/cockroachdb-build"
    tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
      kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/cockroachdb-build -xf -
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "cd /tmp/cockroachdb-build && mvn package -DskipTests -q --batch-mode"
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/cockroachdb-build/target/${PLUGIN_JAR_NAME}" \
  ...
  if [[ -n "$APP_LABEL" ]]; then
    DEPLOY_PODS=$(kubectl get pods "${NS_FLAG[@]}" -l "app=$APP_LABEL" \
      --field-selector=status.phase=Running \
      -o jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' 2>/dev/null | grep -v '^$' || echo "$TARGET")
  fi
...
echo "  2. Go to Sources → Add Source → CockroachDB"
echo "  3. Enter your host, port (26257), database, username, and password"
echo "  4. Click Save — your CockroachDB tables will appear in the catalog"
echo ""
echo -e "${BOLD}Example queries:${RESET}"
echo "  SELECT * FROM cockroach_source.public.my_table LIMIT 10;"
echo "  SELECT region, COUNT(*), AVG(amount) FROM cockroach_source.public.orders"
