#!/usr/bin/env bash
# ============================================================================
# Dremio Microsoft Dataverse Connector — Version-aware Rebuild Script
#
# PURPOSE
...
#
# WHAT IT DOES
#   1. Detects the Dremio, Arrow, and Calcite JAR versions from the running instance
...
PLUGIN_JAR_NAME="dremio-dataverse-connector-1.0.0.jar"
...
echo -e "${BOLD}${CYAN}   Dremio Dataverse Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
if [[ -n "$DETECTED_CALCITE" ]]; then
  CURRENT_CALCITE=$(sed -n 's|.*<calcite.version>\([^<]*\).*|\1|p' "$POM_XML" | head -1 || echo "")
  if [[ -n "$CURRENT_CALCITE" ]]; then
  ...
  if [[ "$MODE" == "docker" ]]; then
    docker exec "$TARGET" bash -c "
      mvn install:install-file -q \
        -Dfile='${jar_path}' -DgroupId='${group_id}' \
        -DartifactId='${artifact_id}' -Dversion='${version}' \
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" mkdir -p /tmp/dataverse-rebuild
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/dataverse-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/dataverse-rebuild/"

  if ! docker exec "$TARGET" bash -c "
    cd /tmp/dataverse-rebuild
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/dataverse-rebuild && mkdir -p /tmp/dataverse-rebuild"
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/dataverse-rebuild -xf -
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/dataverse-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  if ! $BUILD_FAILED; then
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/dataverse-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c \
    "cp /tmp/dataverse-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/"
  docker cp "${TARGET}:/tmp/dataverse-rebuild/target/${PLUGIN_JAR_NAME}" \
