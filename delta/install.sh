#!/bin/bash
# ============================================================================
# Dremio Delta Lake Connector — Installer
#
...
banner() {
  ...
  echo -e "${BOLD}${CYAN}   Dremio Delta Lake Connector Installer${RESET}"
  echo -e "${BOLD}${CYAN}════════════════════════════════════════════════${RESET}"
  echo ""
}
...
'
  ...
  if [[ "$exec_cmd" == docker* ]]; then
    ...
    docker exec -u root "${container_name}" bash -c "rm -rf /tmp/delta-build && mkdir -p /tmp/delta-build"
    docker cp "${SCRIPT_DIR}/." "${container_name}:/tmp/delta-build/"
    docker exec -u root "${container_name}" bash -c "rm -rf /tmp/delta-build/target && chown -R dremio:dremio /tmp/delta-build"
    docker exec "${container_name}" bash -c "
      cd /tmp/delta-build && mvn package -DskipTests -q
      echo 'Build complete.'
      ls -lh target/${PLUGIN_JAR_NAME}
    "
    # Pull the freshly-built JAR back to host jars/ for next time
    docker cp "${container_name}:/tmp/delta-build/target/${PLUGIN_JAR_NAME}" \
      "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}" 2>/dev/null && \
      ok "Built JAR saved to jars/ for future --prebuilt installs"
    DEPLOY_FROM="/tmp/delta-build/target/${PLUGIN_JAR_NAME}"
  else
    # kubectl: use tar pipe
    tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
      kubectl exec -i "$TARGET" -- tar -C /tmp/delta-build -xf -
    $exec_cmd bash -c "cd /tmp/delta-build && mvn package -DskipTests -q"
    DEPLOY_FROM="/tmp/delta-build/target/${PLUGIN_JAR_NAME}"
  fi
  ok "Build successful"
}
...
'
    ...
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "rm -rf /tmp/delta-build && mkdir -p /tmp/delta-build"
    tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
      kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/delta-build -xf -
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "cd /tmp/delta-build && mvn package -DskipTests -q --batch-mode"
    ok "Build successful"

    step "3d" "Extracting built JAR to local jars/..."
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/delta-build/target/${PLUGIN_JAR_NAME}" \
...
echo "  2. Go to Sources → Add Source → Delta Lake"
echo "  3. Set the root path to your Delta tables directory"
echo "  4. Click Save — your Delta tables will be auto-discovered"
echo ""
echo -e "${BOLD}Example queries:${RESET}"
echo "  -- Read from a Delta table"
echo "  SELECT * FROM delta_source.my_table LIMIT 10;"
echo ""
echo "  -- Write a new Delta table via CTAS"
echo "  -- IMPORTANT: use 'delta_write', NOT 'delta'"
echo "  CREATE TABLE delta_source.output.my_table"
echo "  STORE AS (type => 'delta_write')"
echo "  AS SELECT * FROM other_source.my_data;"
echo ""
echo -e "${BOLD}Note:${RESET} Always use STORE AS (type => 'delta_write') for writes."
echo "  Using 'delta' routes to Dremio's built-in read-only plugin."
echo ""
