#!/usr/bin/env bash
# =============================================================================
# Build script for Dremio Delta Lake Connector Kubernetes images
#
# Builds two Docker images:
#   1. Full custom Dremio image with Delta plugin baked in (recommended)
...
if [[ -n "$REGISTRY" ]]; then
  FULL_IMAGE="${REGISTRY}/dremio-with-delta:${DREMIO_TAG}"
  INIT_IMAGE="${REGISTRY}/dremio-delta-plugin-init:${PLUGIN_VERSION}"
else
  FULL_IMAGE="dremio-with-delta:${DREMIO_TAG}"
  INIT_IMAGE="dremio-delta-plugin-init:${PLUGIN_VERSION}"
fi

echo "=== Dremio Delta Lake Connector — Kubernetes Image Builder ==="
...
if [[ "$BUILD_INIT" == true ]]; then
  ...
  if ! ls "${SCRIPT_DIR}/target/dremio-delta-connector-"*"-plugin.jar" &>/dev/null; then
    echo ""
