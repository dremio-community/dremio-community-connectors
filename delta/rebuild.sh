#!/usr/bin/env bash
# ============================================================================
# Dremio Delta Lake Connector — Version-aware Rebuild Script
#
# PURPOSE
...
PLUGIN_JAR_NAME="dremio-delta-connector-1.0.0-SNAPSHOT-plugin.jar"
...
echo -e "${BOLD}${CYAN}   Dremio Delta Lake Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
CURRENT_HADOOP=$(sed -n 's|.*<hadoop.version>\([^<]*\)</hadoop.version>.*|\1|p' "$POM_XML" | head -1 || echo "")
...
# Hadoop JARs (Delta reads Parquet files via Hadoop FileSystem)
...
# protobuf-java 3.25.5 — standard Maven Central artifact; fetched automatically by mvn.
# If behind an air-gapped network, pre-install with:
#   mvn install:install-file -Dfile=protobuf-java-3.25.5.jar \
#     -DgroupId=com.google.protobuf -DartifactId=protobuf-java \
#     -Dversion=3.25.5 -Dpackaging=jar
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c "rm -rf /tmp/delta-rebuild && mkdir -p /tmp/delta-rebuild"
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/delta-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/delta-rebuild/"
  docker exec -u root "$TARGET" bash -c "chmod -R 777 /tmp/delta-rebuild"

  if ! docker exec "$TARGET" bash -c "
    cd /tmp/delta-rebuild
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/delta-rebuild && mkdir -p /tmp/delta-rebuild"
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/delta-rebuild -xf -
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/delta-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  if ! $BUILD_FAILED; then
    ...
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/delta-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if $BUILD_FAILED; then
  ...
  echo    "    • FileSystemPlugin method signature changed (Delta extends it)"
  echo    ""
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c \
    "cp /tmp/delta-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/"
  docker cp "${TARGET}:/tmp/delta-rebuild/target/${PLUGIN_JAR_NAME}" \
