#!/usr/bin/env bash
# ============================================================================
# Dremio Pinecone Connector — Version-aware Rebuild Script
#
# Detects the exact Dremio version running in the target environment,
# updates pom.xml to match, rebuilds the connector JAR, and redeploys.
#
# USAGE
...
echo -e "${BOLD}${CYAN}   Dremio Pinecone Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
install_jar "${JARS}/dremio-services-namespace-${DV}.jar" com.dremio.service dremio-services-namespace "$DV"
...
BUILD_FAILED=false
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c "rm -rf /tmp/pinecone-rebuild && mkdir -p /tmp/pinecone-rebuild"
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/pinecone-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/pinecone-rebuild/"
  docker exec -u root "$TARGET" bash -c "chmod -R 777 /tmp/pinecone-rebuild"
  if ! docker exec "$TARGET" bash -c \
    "cd /tmp/pinecone-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/pinecone-rebuild && mkdir -p /tmp/pinecone-rebuild"
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/pinecone-rebuild -xf -
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/pinecone-rebuild && mvn package -DskipTests -q --batch-mode"; then
    BUILD_FAILED=true
  fi
fi
...
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c \
    "cp /tmp/pinecone-rebuild/jars/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/ && \
     chown dremio:dremio /opt/dremio/jars/3rdparty/${PLUGIN_JAR_NAME}"
  docker cp "${TARGET}:/tmp/pinecone-rebuild/jars/${PLUGIN_JAR_NAME}" \
    "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}" 2>/dev/null && \
    ok "Saved plugin JAR to jars/ for future --prebuilt installs"
elif [[ "$MODE" == "local" ]]; then
  cp "${BUILD_TMPDIR}/jars/${PLUGIN_JAR_NAME}" "${TARGET}/jars/3rdparty/"
  cp "${BUILD_TMPDIR}/jars/${PLUGIN_JAR_NAME}" "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}" 2>/dev/null || true
  rm -rf "$BUILD_TMPDIR"
else
