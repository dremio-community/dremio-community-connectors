#!/bin/bash
# ============================================================================
# Dremio Pinecone Connector — Installer
#
...
PLUGIN_JAR_NAME="dremio-pinecone-connector-1.0.0.jar"
...
banner() {
  ...
  echo -e "${BOLD}${CYAN}   Dremio Pinecone Connector Installer${RESET}"
  echo -e "${BOLD}${CYAN}════════════════════════════════════════════════${RESET}"
  echo ""
}
...
if [[ "$MODE" == "docker" ]]; then
  ...
  if [[ "$JAR_MODE" == "prebuilt" ]]; then
    ...
    docker exec -u root "$TARGET" bash -c "rm -rf /tmp/pinecone-build && mkdir -p /tmp/pinecone-build"
    docker cp "${SCRIPT_DIR}/." "${TARGET}:/tmp/pinecone-build/"
    docker exec -u root "$TARGET" bash -c "rm -rf /tmp/pinecone-build/target && chown -R dremio:dremio /tmp/pinecone-build"
    docker exec "$TARGET" bash -c "cd /tmp/pinecone-build && mvn package -DskipTests -q --batch-mode"
    ok "Build successful"
    docker cp "${TARGET}:/tmp/pinecone-build/jars/${PLUGIN_JAR_NAME}" \
      "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}" 2>/dev/null && \
      ok "Built JAR saved to jars/ for future --prebuilt installs"
    DEPLOY_FROM="/tmp/pinecone-build/jars/${PLUGIN_JAR_NAME}"
  fi
...
# ============================================================================
  ...
  if [[ "$JAR_MODE" == "prebuilt" ]]; then
    ...
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "rm -rf /tmp/pinecone-build && mkdir -p /tmp/pinecone-build"
    tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
      kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/pinecone-build -xf -
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "cd /tmp/pinecone-build && mvn package -DskipTests -q --batch-mode"
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/pinecone-build/jars/${PLUGIN_JAR_NAME}" \
...
echo "  2. Go to Sources → Add Source → Pinecone"
echo "  3. Enter your Pinecone API key"
echo "  4. Click Save — your Pinecone indexes will appear as tables"
echo ""
echo -e "${BOLD}Example queries:${RESET}"
echo "  SELECT * FROM pinecone_source.my_index LIMIT 10;"
echo "  SELECT _id, price, category FROM pinecone_source.products"
echo "    WHERE category = 'electronics' ORDER BY price DESC;"
echo ""
