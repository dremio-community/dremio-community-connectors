#!/usr/bin/env bash
# install.sh — Deploy the Dremio Prometheus connector to a running Dremio instance
#
# Usage:
#   ./install.sh [OPTIONS]
#
...
if $PREBUILT; then
  JAR_PATH=$(find "$SCRIPT_DIR/jars" -name "dremio-prometheus-connector-*.jar" | head -1)
  ...
  if [[ -n "$CONTAINER" ]]; then
    ...
    $DOCKER exec -u root "$CONTAINER" bash -c "rm -rf /tmp/prometheus-build" 2>/dev/null || true
    $DOCKER cp "$SCRIPT_DIR/." "${CONTAINER}:/tmp/prometheus-build"
    $DOCKER exec -u root "$CONTAINER" bash -c "cd /tmp/prometheus-build && mvn package -q -DskipTests"
    $DOCKER cp "${CONTAINER}:/tmp/prometheus-build/jars/dremio-prometheus-connector-1.0.0.jar" \
               "${SCRIPT_DIR}/jars/dremio-prometheus-connector-1.0.0.jar" 2>/dev/null || true
    JAR_PATH="${SCRIPT_DIR}/jars/dremio-prometheus-connector-1.0.0.jar"
  else
...
if [[ -n "$CONTAINER" ]]; then
  ...
  $DOCKER exec -u root "$CONTAINER" bash -c "cp /dev/stdin ${DREMIO_JAR_DIR}/${JAR_NAME}" < "$JAR_PATH"
...
echo ""
ok "Prometheus connector installed successfully"
echo ""
echo "  Test with:"
echo "    SELECT * FROM prometheus_source.targets;"
echo "    SELECT metric_name, type, help FROM prometheus_source.metrics LIMIT 20;"
echo "    SELECT metric_name, labels, timestamp, value FROM prometheus_source.samples;"
echo ""
