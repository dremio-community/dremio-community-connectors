...
public class PrometheusScanDrel extends ScanRelBase implements Rel {

    private final PrometheusScanSpec scanSpec;

    public PrometheusScanDrel(RelOptCluster cluster, RelTraitSet traitSet, RelOptTable table,
            StoragePluginId pluginId, TableMetadata tableMetadata,
            List<SchemaPath> projectedColumns, double observedRowcountAdjustment,
            List<RelHint> hints, PrometheusScanSpec scanSpec) {
    ...
    public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
        return new PrometheusScanDrel(getCluster(), traitSet, getTable(), getPluginId(),
    ...
    public ScanRelBase cloneWithProject(List<SchemaPath> columns) {
        return new PrometheusScanDrel(getCluster(), getTraitSet(), getTable(), getPluginId(),
