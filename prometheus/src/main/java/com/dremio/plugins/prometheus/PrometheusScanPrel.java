...
public class PrometheusScanPrel extends ScanPrelBase implements OldScanPrelBase {

    private final PrometheusGroupScan groupScan;
    private final PrometheusScanSpec scanSpec;

    public PrometheusScanPrel(RelOptCluster cluster, RelTraitSet traitSet, RelOptTable table,
            StoragePluginId pluginId, TableMetadata tableMetadata,
            List<SchemaPath> projectedColumns, double observedRowcountAdjustment,
            List<RelHint> hints, PrometheusGroupScan groupScan, PrometheusScanSpec scanSpec) {
    ...
    public PhysicalOperator getPhysicalOperator(PhysicalPlanCreator creator) throws IOException {
        return new PrometheusGroupScan(creator.props(this, null, getTableMetadata().getSchema()),
                getTableMetadata(), getProjectedColumns(), getPluginId(), scanSpec);
    }

    @Override
    public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
        return new PrometheusScanPrel(getCluster(), traitSet, getTable(), getPluginId(),
    ...
    public ScanRelBase cloneWithProject(List<SchemaPath> columns) {
        return new PrometheusScanPrel(getCluster(), getTraitSet(), getTable(), getPluginId(),
