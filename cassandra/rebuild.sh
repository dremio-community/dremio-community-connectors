#!/usr/bin/env bash
# ============================================================================
# Dremio Cassandra Connector — Version-aware Rebuild Script
#
# PURPOSE
...
#
# WHAT IT DOES
#   1. Detects the Dremio, Arrow, and Hadoop JAR versions from the running instance
...
echo -e "${BOLD}${CYAN}   Dremio Cassandra Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
# Detect Dremio version from dremio-common JAR filename
...
if [[ -n "$HADOOP_JAR" ]]; then
  ...
  DETECTED_HADOOP=""
  warn "Hadoop JAR not found in 3rdparty/ — will skip Hadoop version update"
fi

# Detect Calcite version
...
CURRENT_DREMIO=$(grep -oP '(?<=<dremio.version>)[^<]+' "$POM_XML" | head -1)
CURRENT_ARROW=$(grep -oP '(?<=<arrow.version>)[^<]+' "$POM_XML" | head -1 || echo "")
CURRENT_HADOOP=$(grep -oP '(?<=<hadoop.version>)[^<]+' "$POM_XML" | head -1 || echo "")
...
# Update hadoop.version if present in pom.xml and detected
if [[ -n "$DETECTED_HADOOP" && -n "$CURRENT_HADOOP" ]]; then
...
if [[ -n "$DETECTED_HADOOP" ]]; then
  install_jar_from_target "${TP}/hadoop-common-${DETECTED_HADOOP}.jar" org.apache.hadoop hadoop-common "$DETECTED_HADOOP"
fi
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" mkdir -p /tmp/cassandra-rebuild
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/cassandra-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/cassandra-rebuild/"

  if ! docker exec "$TARGET" bash -c "
    cd /tmp/cassandra-rebuild
    mvn package -DskipTests --batch-mode 2>&1
  "; then
    BUILD_FAILED=true
  fi
  ...
  if ! (cd "$BUILD_TMPDIR" && mvn package -DskipTests --batch-mode 2>&1); then
    BUILD_FAILED=true
  fi
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/cassandra-rebuild && mkdir -p /tmp/cassandra-rebuild"
  # Copy source into the pod
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/cassandra-rebuild -xf -
  # Build
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/cassandra-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  # Extract the built JAR back to the local jars/ directory so we can
  # distribute it to all pods (coordinator + executors) in Step 6.
  if ! $BUILD_FAILED; then
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/cassandra-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if $BUILD_FAILED; then
  ...
  echo    "    2. Fix the affected Java file(s) — usually 1-3 files in planning/"
  echo    "    3. Re-run: ./rebuild.sh --force"
  echo    ""
  echo    "  pom.xml has been updated. Your source changes (if any) will be picked up."
  echo    "  pom.xml.bak contains the previous version string if you need to roll back."
  echo ""
  # Restore pom.xml to avoid leaving it in a broken state
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c \
    "cp /tmp/cassandra-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/"
  # Pull the freshly-built JAR back to host jars/ so it can be used as prebuilt next time
  docker cp "${TARGET}:/tmp/cassandra-rebuild/target/${PLUGIN_JAR_NAME}" \
  ...
  # Auto-discover all Dremio pods (coordinator + executors) matching the same label
...
if [[ "$MODE" == "docker" ]]; then
  ...
  echo ""
  # Try to auto-detect the StatefulSet name from the pod selector
  SS_NAME=""
  if [[ -n "${SELECTOR:-}" ]]; then
