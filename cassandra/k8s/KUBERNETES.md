# Dremio Cassandra Connector — Kubernetes Install Guide

This guide covers deploying the Cassandra connector to **Dremio Enterprise** running on
...
From the `dremio-cassandra-connector/` directory:
...
This builds `your-registry.example.com/dremio-with-cassandra:26.0.5` and pushes it.
...
add the Cassandra plugin JAR into that volume.
...
This builds `your-registry.example.com/dremio-cassandra-plugin-init:1.0.0` and pushes it.
...
- `cassandra-plugin` init container: set to the init image you just pushed
...
## Registering the Cassandra Source

After Dremio is running with the plugin installed, register a Cassandra source via the
Dremio REST API (or the UI under **Sources → + → Cassandra**):
...
curl -s -X POST https://dremio.example.com/api/v3/catalog \
  ...
  -d '{
    ...
    "type": "CASSANDRA",
    "config": {
      "contactPoints": "cassandra.example.com",
      "port": 9042,
      "username": "cassandra",
...
kubectl get pods -n dremio -l role=dremio-cluster-pod \
  ...
  ls /opt/dremio/jars/3rdparty/dremio-cassandra-connector-*.jar

# Run a quick smoke test
curl -s -X POST https://dremio.example.com/api/v3/sql \
  -H "Authorization: _dremio${TOKEN}" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT * FROM cassandra.mykeyspace.mytable LIMIT 5"}' \
  | jq '.id'
```
...
|---|---|---|
| Plugin JAR not found in pod | Init container failed or image not pulled | `kubectl logs -n dremio <pod> -c cassandra-plugin` |
| `ClassNotFoundException` for Cassandra classes | JAR deployed to wrong path | Verify JAR is in `/opt/dremio/jars/3rdparty/`, not `/opt/dremio/jars/` |
| Connection refused to Cassandra | Wrong contact point or port | Verify `contactPoints` and `port` (default 9042) in source config |
| Source shows as unhealthy after deploy | Plugin loaded but Cassandra unreachable | Check network policy allows egress to Cassandra port 9042 |
...
| Authentication failure | Wrong credentials or auth not enabled | Verify `username`/`password` match Cassandra's authenticator config |
