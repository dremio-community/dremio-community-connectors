#!/usr/bin/env bash
# =============================================================================
# Build script for Dremio Cassandra Connector Kubernetes images
#
# Builds two Docker images:
#   1. Full custom Dremio image with Cassandra plugin baked in (recommended)
...
if [[ -n "$REGISTRY" ]]; then
  FULL_IMAGE="${REGISTRY}/dremio-with-cassandra:${DREMIO_TAG}"
  INIT_IMAGE="${REGISTRY}/dremio-cassandra-plugin-init:${PLUGIN_VERSION}"
else
  FULL_IMAGE="dremio-with-cassandra:${DREMIO_TAG}"
  INIT_IMAGE="dremio-cassandra-plugin-init:${PLUGIN_VERSION}"
fi

echo "=== Dremio Cassandra Connector — Kubernetes Image Builder ==="
...
if [[ "$BUILD_INIT" == true ]]; then
  ...
  if ! ls "${SCRIPT_DIR}/target/dremio-cassandra-connector-"*"-plugin.jar" &>/dev/null; then
    echo ""
