...
import com.dremio.plugins.servicenow.ServiceNowConnection.ServiceNowField;
import com.dremio.plugins.servicenow.ServiceNowConnection.ServiceNowPage;
...
/**
 * Reads records from ServiceNow REST API pages into Arrow vectors.
 *
 ...
 */
public class ServiceNowRecordReader extends AbstractRecordReader {

    private static final Logger logger = LoggerFactory.getLogger(ServiceNowRecordReader.class);
    private static final ObjectMapper MAPPER = new ObjectMapper();
    private static final int BATCH_SIZE = 100;

    private final ServiceNowConnection connection;
    private final ServiceNowScanSpec spec;
    private final com.dremio.exec.record.BatchSchema schema;
    private final ServiceNowTable table;

    // Projected fields + vectors (populated in setup)
    private List<ServiceNowField> projectedZdFields;
    private List<ValueVector> vectors;

    // Pagination state
    private ServiceNowPage currentPage;
    ...
    public ServiceNowRecordReader(
            ...
            ServiceNowScanSpec spec,
            com.dremio.exec.record.BatchSchema schema) {
        super(context, null);
        this.connection = connection;
        this.spec = spec;
        this.schema = schema;
        this.table = ServiceNowConnection.getTable(spec.getTableName());
    }
    ...
    public void setup(OutputMutator mutator) throws ExecutionSetupException {
        ...
        // Build a name→ServiceNowField map for fast lookup
        Map<String, ServiceNowField> zdFieldByName = new java.util.LinkedHashMap<>();
        for (ServiceNowField zf : table.fields) {
        ...
        for (Field field : schema.getFields()) {
            ...
            if (v == null) continue;
            ServiceNowField zf = zdFieldByName.get(field.getName());
            if (zf == null) continue;
        ...
        logger.debug("ServiceNowRecordReader setup: table={}", spec.getTableName());
    }
    ...
    public int next() {
        ...
        try {
            ...
            logger.error("Error reading from ServiceNow", e);
            throw new RuntimeException("Failed to read from ServiceNow: " + e.getMessage(), e);
        }
    }

    @Override
    public void close() throws Exception {
        logger.debug("ServiceNowRecordReader closed for table '{}'", spec.getTableName());
    }
    ...
    private void writeRecord(Map<String, Object> record, int idx) {
        for (int i = 0; i < projectedZdFields.size(); i++) {
            ServiceNowField zf = projectedZdFields.get(i);
    ...
    private void writeValue(ValueVector vector, ArrowType type, Object value, int idx) {
        if (type instanceof ArrowType.Utf8) {
            ...
            // ServiceNow timestamps: "2024-01-15 10:30:00" (UTC)
