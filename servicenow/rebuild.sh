#!/usr/bin/env bash
# ============================================================================
# Dremio ServiceNow Connector — Version-aware Rebuild Script
#
# PURPOSE
...
PLUGIN_JAR_NAME="dremio-servicenow-connector-1.0.0.jar"
...
echo -e "${BOLD}${CYAN}   Dremio ServiceNow Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" mkdir -p /tmp/servicenow-rebuild
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/servicenow-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/servicenow-rebuild/"

  if ! docker exec "$TARGET" bash -c "
    cd /tmp/servicenow-rebuild
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/servicenow-rebuild && mkdir -p /tmp/servicenow-rebuild"
  # Copy source into the pod
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/servicenow-rebuild -xf -
  # Build
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/servicenow-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  if ! $BUILD_FAILED; then
    ...
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/servicenow-rebuild/jars/${PLUGIN_JAR_NAME}" \
...
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c \
    "cp /tmp/servicenow-rebuild/jars/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/"
  # Pull the freshly-built JAR back to host jars/ so it can be used as prebuilt next time
  docker cp "${TARGET}:/tmp/servicenow-rebuild/jars/${PLUGIN_JAR_NAME}" \
