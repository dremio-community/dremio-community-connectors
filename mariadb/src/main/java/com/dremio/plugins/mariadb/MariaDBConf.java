...
/**
 * Source configuration for the Dremio MariaDB Connector.
 *
 * <p>Uses Dremio's ARP (Advanced Relational Pushdown) framework over the official
 * MariaDB Connector/J JDBC driver ({@code org.mariadb.jdbc:mariadb-java-client}).
 * The ARP YAML dialect file ({@code mariadb-arp.yaml}) drives all SQL translation,
 * predicate pushdown, aggregation pushdown, and function mapping.</p>
 *
 * <p>Connects to MariaDB on the standard port 3306 (TCP).</p>
 */
@SourceType(value = "MARIADB", label = "MariaDB", uiConfig = "mariadb-layout.json")
public class MariaDBConf extends AbstractArpConf<MariaDBConf> {

  private static final String ARP_FILENAME = "arp/implementation/mariadb-arp.yaml";
  private static final String DRIVER = "org.mariadb.jdbc.Driver";

  private static final ArpDialect ARP_DIALECT =
      AbstractArpConf.loadArpFile(ARP_FILENAME, MariaDBDialect::new);
  ...
  /**
   * Comma-separated list of MariaDB schema (database) names to exclude from
   * the Dremio catalog browser. Always hidden in addition to information_schema,
   * performance_schema, mysql, and sys.
   * Example: "staging,temp_db,audit_log"
   */
  @Tag(12)
  ...
  /**
   * Additional JDBC driver properties to pass to the MariaDB Connector/J driver.
  ...
  public JdbcPluginConfig buildPluginConfig(
    ...
    JdbcPluginConfig.Builder builder = configBuilder
        ...
        .clearHiddenSchemas()
        // Hide MariaDB system databases from the catalog.
        .addHiddenSchema("information_schema")
        .addHiddenSchema("performance_schema")
        .addHiddenSchema("mysql")
        .addHiddenSchema("sys");
  ...
  private Properties buildDriverProperties() {
    ...
    // Use UTC for session timezone to avoid daylight-saving surprises
  ...
  /**
   * Builds the JDBC connection URL for MariaDB.
   *
   * <p>Format: {@code jdbc:mariadb://host:port/database}
   *
   * <p>If no database is specified, connects to the server without selecting a default schema
   * so all schemas are visible through catalog browsing.
   */
  @VisibleForTesting
  public String toJdbcConnectionString() {
    ...
    final String db = (database != null && !database.trim().isEmpty())
        ? "/" + database.trim() : "";
    return String.format("jdbc:mariadb://%s:%d%s", h, port, db);
  }
}
