#!/bin/bash
# ============================================================================
# Dremio MariaDB Connector — Installer
#
...
PLUGIN_JAR_NAME="dremio-mariadb-connector-1.0.0-SNAPSHOT-plugin.jar"
PREBUILT_JAR="${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}"
DREMIO_JAR_SUBDIR="jars/3rdparty"

# MariaDB Connector/J — single JAR, no separate driver deployment needed.
...
banner() {
  ...
  echo -e "${BOLD}${CYAN}   Dremio MariaDB Connector Installer${RESET}"
  echo -e "${BOLD}${CYAN}════════════════════════════════════════════════${RESET}"
  echo ""
}
...
if [[ "$MODE" == "docker" ]]; then
  ...
  if [[ "$JAR_MODE" == "prebuilt" ]]; then
    ...
    docker exec -u root "$TARGET" bash -c "rm -rf /tmp/mariadb-build && mkdir -p /tmp/mariadb-build"
    docker cp "${SCRIPT_DIR}/." "${TARGET}:/tmp/mariadb-build/"
    docker exec -u root "$TARGET" bash -c "rm -rf /tmp/mariadb-build/target && chown -R dremio:dremio /tmp/mariadb-build"
    docker exec "$TARGET" bash -c "cd /tmp/mariadb-build && mvn package -DskipTests -q --batch-mode"
    ok "Build successful"
    docker cp "${TARGET}:/tmp/mariadb-build/target/${PLUGIN_JAR_NAME}" \
      "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}" 2>/dev/null && \
      ok "Built JAR saved to jars/ for future --prebuilt installs"
    DEPLOY_FROM="/tmp/mariadb-build/target/${PLUGIN_JAR_NAME}"
  fi
...
# ============================================================================
  ...
  if [[ "$JAR_MODE" == "prebuilt" ]]; then
    ...
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "rm -rf /tmp/mariadb-build && mkdir -p /tmp/mariadb-build"
    tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
      kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/mariadb-build -xf -
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "cd /tmp/mariadb-build && mvn package -DskipTests -q --batch-mode"
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/mariadb-build/target/${PLUGIN_JAR_NAME}" \
...
echo "  2. Go to Sources → Add Source → MariaDB"
echo "  3. Enter your host, port (3306), database, username, and password"
echo "  4. Click Save — your MariaDB tables will appear in the catalog"
echo ""
echo -e "${BOLD}Example queries:${RESET}"
echo "  SELECT * FROM mariadb_source.my_database.my_table LIMIT 10;"
echo "  SELECT region, COUNT(*), AVG(amount) FROM mariadb_source.sales.orders"
