#!/usr/bin/env bash
# ============================================================================
# Dremio Kafka Connector — Version-aware Rebuild Script
#
# PURPOSE
...
echo -e "${BOLD}${CYAN}   Dremio Apache Kafka Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
# Guava and javax.inject (stable versions, just ensure they're present)
install_jar_from_target "${TP}/guava-33.4.0-jre.jar"  com.google.guava  guava        "33.4.0-jre" 2>/dev/null || true
install_jar_from_target "${TP}/javax.inject-1.jar"     javax.inject      javax.inject "1"          2>/dev/null || true
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" mkdir -p /tmp/kafka-rebuild
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/kafka-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/kafka-rebuild/"
  docker exec -u root "$TARGET" bash -c "chmod -R 777 /tmp/kafka-rebuild"

  if ! docker exec "$TARGET" bash -c "
    cd /tmp/kafka-rebuild
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/kafka-rebuild && mkdir -p /tmp/kafka-rebuild"
  # Copy source into the pod
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/kafka-rebuild -xf -
  # Build
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/kafka-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  if ! $BUILD_FAILED; then
    ...
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/kafka-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c \
    "cp /tmp/kafka-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/"
  docker cp "${TARGET}:/tmp/kafka-rebuild/target/${PLUGIN_JAR_NAME}" \
