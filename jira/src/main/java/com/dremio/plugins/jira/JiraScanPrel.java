...
public class JiraScanPrel extends ScanPrelBase implements OldScanPrelBase {

    private final JiraGroupScan groupScan;
    private final JiraScanSpec scanSpec;

    public JiraScanPrel(RelOptCluster cluster, RelTraitSet traitSet, RelOptTable table,
            StoragePluginId pluginId, TableMetadata tableMetadata,
            List<SchemaPath> projectedColumns, double observedRowcountAdjustment,
            List<RelHint> hints, JiraGroupScan groupScan, JiraScanSpec scanSpec) {
    ...
    public PhysicalOperator getPhysicalOperator(PhysicalPlanCreator creator) throws IOException {
        return new JiraGroupScan(creator.props(this, null, getTableMetadata().getSchema()),
                getTableMetadata(), getProjectedColumns(), getPluginId(), scanSpec);
    }

    @Override
    public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
        return new JiraScanPrel(getCluster(), traitSet, getTable(), getPluginId(),
    ...
    public ScanRelBase cloneWithProject(List<SchemaPath> columns) {
        return new JiraScanPrel(getCluster(), getTraitSet(), getTable(), getPluginId(),
