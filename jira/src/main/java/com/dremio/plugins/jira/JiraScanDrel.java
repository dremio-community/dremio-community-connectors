...
public class JiraScanDrel extends ScanRelBase implements Rel {

    private final JiraScanSpec scanSpec;

    public JiraScanDrel(RelOptCluster cluster, RelTraitSet traitSet, RelOptTable table,
            StoragePluginId pluginId, TableMetadata tableMetadata,
            List<SchemaPath> projectedColumns, double observedRowcountAdjustment,
            List<RelHint> hints, JiraScanSpec scanSpec) {
        super(cluster, traitSet, table, pluginId, tableMetadata,
                projectedColumns, observedRowcountAdjustment, hints);
        this.scanSpec = scanSpec;
    }

    public JiraScanSpec getScanSpec() { return scanSpec; }

    @Override
    public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
        return new JiraScanDrel(getCluster(), traitSet, getTable(), getPluginId(),
    ...
    public ScanRelBase cloneWithProject(List<SchemaPath> columns) {
        return new JiraScanDrel(getCluster(), getTraitSet(), getTable(), getPluginId(),
