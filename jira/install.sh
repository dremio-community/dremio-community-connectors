#!/usr/bin/env bash
# install.sh — Deploy the Dremio Jira connector to a running Dremio instance
#
# Usage:
#   ./install.sh [OPTIONS]
#
...
if $PREBUILT; then
  JAR_PATH=$(find "$SCRIPT_DIR/jars" -name "dremio-jira-connector-*.jar" | head -1)
  ...
  if [[ -n "$CONTAINER" ]]; then
    ...
    $DOCKER exec -u root "$CONTAINER" bash -c "rm -rf /tmp/jira-build" 2>/dev/null || true
    $DOCKER cp "$SCRIPT_DIR/." "${CONTAINER}:/tmp/jira-build"
    $DOCKER exec -u root "$CONTAINER" bash -c "cd /tmp/jira-build && mvn package -q -DskipTests"
    $DOCKER cp "${CONTAINER}:/tmp/jira-build/jars/dremio-jira-connector-1.0.0.jar" \
               "${SCRIPT_DIR}/jars/dremio-jira-connector-1.0.0.jar" 2>/dev/null || true
    JAR_PATH="${SCRIPT_DIR}/jars/dremio-jira-connector-1.0.0.jar"
  else
...
echo ""
ok "Jira connector installed successfully"
echo ""
echo "  Test with:"
echo "    SELECT key, summary, status, assignee_display_name FROM jira_source.issues LIMIT 10;"
echo "    SELECT key, name, project_type_key FROM jira_source.projects;"
echo "    SELECT * FROM jira_source.priorities;"
echo ""
