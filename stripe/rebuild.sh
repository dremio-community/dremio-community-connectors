#!/usr/bin/env bash
# ============================================================================
# Dremio Stripe Connector — Version-aware Rebuild Script
#
# PURPOSE
...
PLUGIN_JAR_NAME="dremio-stripe-connector-1.0.0.jar"
...
echo -e "${BOLD}${CYAN}   Dremio Stripe Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
if [[ "$MODE" == "docker" ]]; then
  ...
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- which mvn > /dev/null 2>&1; then
    ...
    info "Build locally and use: ./install.sh --k8s $TARGET --prebuilt"
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" mkdir -p /tmp/stripe-rebuild
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/stripe-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/stripe-rebuild/"

  if ! docker exec "$TARGET" bash -c "
    cd /tmp/stripe-rebuild
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/stripe-rebuild && mkdir -p /tmp/stripe-rebuild"
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/stripe-rebuild -xf -
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/stripe-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  if ! $BUILD_FAILED; then
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/stripe-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c \
    "cp /tmp/stripe-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/"
  docker cp "${TARGET}:/tmp/stripe-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if [[ "$MODE" == "docker" ]]; then
  ...
  warn "For persistence, mount jars/3rdparty/ from a PVC."
fi
