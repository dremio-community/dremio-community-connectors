#!/usr/bin/env bash
# ============================================================================
# Dremio Redis Connector — Version-aware Rebuild Script
#
...
echo -e "${BOLD}${CYAN}   Dremio Redis Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
BUILD_FAILED=false
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c "rm -rf /tmp/redis-rebuild && mkdir -p /tmp/redis-rebuild"
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/redis-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/redis-rebuild/"
  docker exec -u root "$TARGET" bash -c "chmod -R 777 /tmp/redis-rebuild"
  if ! docker exec "$TARGET" bash -c "
    cd /tmp/redis-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/redis-rebuild && mkdir -p /tmp/redis-rebuild"
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/redis-rebuild -xf -
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/redis-rebuild && mvn package -DskipTests -q --batch-mode"; then
    BUILD_FAILED=true
  fi
  if ! $BUILD_FAILED; then
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/redis-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c \
    "cp /tmp/redis-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/"
  docker cp "${TARGET}:/tmp/redis-rebuild/target/${PLUGIN_JAR_NAME}" \
