#!/usr/bin/env bash
# Dremio Pub/Sub Connector — Version-aware Rebuild Script
#
# USAGE
#   ./rebuild.sh [--docker CONTAINER] [--local DREMIO_HOME] [--force] [--dry-run]
#
# EXAMPLES
#   ./rebuild.sh                         # Docker, auto-detects try-dremio
...
#   ./rebuild.sh --dry-run               # Detect version, no changes
set -euo pipefail
export PATH="/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin:$PATH"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
POM_XML="${SCRIPT_DIR}/pom.xml"
PLUGIN_JAR_NAME="dremio-pubsub-connector-1.0.0-SNAPSHOT-plugin.jar"
...
echo -e "\n${BOLD}${CYAN}══════════════════════════════════════════════════${RESET}"
echo -e "${BOLD}${CYAN}   Dremio Google Pub/Sub Connector — Rebuild${RESET}"
...
BUILD_FAILED=false
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" mkdir -p /tmp/pubsub-rebuild
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/pubsub-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/pubsub-rebuild/"
  docker exec -u root "$TARGET" bash -c "chmod -R 777 /tmp/pubsub-rebuild"
  docker exec "$TARGET" bash -c "cd /tmp/pubsub-rebuild && mvn package -DskipTests --batch-mode 2>&1" || BUILD_FAILED=true
else
...
if $BUILD_FAILED; then
  err "Build FAILED — check output above for compilation errors"
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c "cp /tmp/pubsub-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/"
  docker cp "${TARGET}:/tmp/pubsub-rebuild/target/${PLUGIN_JAR_NAME}" "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}"
else
...
echo -e "\n${GREEN}${BOLD}Done.${RESET} Connector: ${PLUGIN_JAR_NAME}"
