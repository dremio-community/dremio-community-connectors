#!/usr/bin/env bash
# ============================================================================
# Dremio Neo4j Connector — Version-aware Rebuild Script
#
...
echo -e "${BOLD}${CYAN}   Dremio Neo4j Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
BUILD_FAILED=false
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c "rm -rf /tmp/neo4j-rebuild && mkdir -p /tmp/neo4j-rebuild"
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/neo4j-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/neo4j-rebuild/"
  docker exec -u root "$TARGET" bash -c "chmod -R 777 /tmp/neo4j-rebuild"
  if ! docker exec "$TARGET" bash -c \
    "cd /tmp/neo4j-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/neo4j-rebuild && mkdir -p /tmp/neo4j-rebuild"
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/neo4j-rebuild -xf -
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/neo4j-rebuild && mvn package -DskipTests -q --batch-mode"; then
    BUILD_FAILED=true
  fi
fi
...
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c \
    "cp /tmp/neo4j-rebuild/jars/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/ && \
     chown dremio:dremio /opt/dremio/jars/3rdparty/${PLUGIN_JAR_NAME}"
  docker cp "${TARGET}:/tmp/neo4j-rebuild/jars/${PLUGIN_JAR_NAME}" \
