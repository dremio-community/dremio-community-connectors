#!/bin/bash
# ============================================================================
# Dremio Neo4j Connector — Installer
#
...
PLUGIN_JAR_NAME="dremio-neo4j-connector-1.0.0.jar"
PREBUILT_JAR="${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}"
DREMIO_JAR_SUBDIR="jars/3rdparty"

# Neo4j Java driver 4.4.x is bundled and shaded — no separate driver deployment needed.
...
banner() {
  ...
  echo -e "${BOLD}${CYAN}   Dremio Neo4j Connector Installer${RESET}"
  echo -e "${BOLD}${CYAN}════════════════════════════════════════════════${RESET}"
  echo ""
}
...
# ============================================================================
# Helper: install Dremio JARs into Maven local repo
# ============================================================================
install_dremio_jars() {
  local exec_cmd="$1"
  $exec_cmd bash -c '
...
install_jar $JARS/dremio-services-namespace-${DREMIO_VER}.jar   com.dremio.service dremio-services-namespace $DREMIO_VER
...
if [[ "$MODE" == "docker" ]]; then
  ...
  if [[ "$JAR_MODE" == "prebuilt" ]]; then
    ...
    docker exec -u root "$TARGET" bash -c "rm -rf /tmp/neo4j-build && mkdir -p /tmp/neo4j-build"
    docker cp "${SCRIPT_DIR}/." "${TARGET}:/tmp/neo4j-build/"
    docker exec -u root "$TARGET" bash -c "rm -rf /tmp/neo4j-build/target && chown -R dremio:dremio /tmp/neo4j-build"
    docker exec "$TARGET" bash -c "cd /tmp/neo4j-build && mvn package -DskipTests -q --batch-mode"
    ok "Build successful"
    docker cp "${TARGET}:/tmp/neo4j-build/jars/${PLUGIN_JAR_NAME}" \
      "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}" 2>/dev/null && \
      ok "Built JAR saved to jars/ for future --prebuilt installs"
    DEPLOY_FROM="/tmp/neo4j-build/jars/${PLUGIN_JAR_NAME}"
  fi
...
# ============================================================================
  ...
  if [[ "$JAR_MODE" == "prebuilt" ]]; then
    ...
    cp "jars/${PLUGIN_JAR_NAME}" "${DREMIO_HOME}/${DREMIO_JAR_SUBDIR}/${PLUGIN_JAR_NAME}"
...
# ============================================================================
  ...
  LOCAL_JAR=""
  if [[ "$JAR_MODE" == "prebuilt" ]]; then
    step "2" "Using pre-built plugin JAR..."
    LOCAL_JAR="$PREBUILT_JAR"
    ok "Plugin JAR ready"
  else
    step "2" "Building from source in pod '$TARGET'..."
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "rm -rf /tmp/neo4j-build && mkdir -p /tmp/neo4j-build"
    tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
      kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/neo4j-build -xf -
    kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
      "cd /tmp/neo4j-build && mvn package -DskipTests -q --batch-mode"
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/neo4j-build/jars/${PLUGIN_JAR_NAME}" \
      "${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}"
    LOCAL_JAR="${SCRIPT_DIR}/jars/${PLUGIN_JAR_NAME}"
    ok "Build successful"
  fi

  step "3" "Deploying JAR to all pods..."
  DEPLOY_PODS=$(kubectl get pods "${NS_FLAG[@]}" \
    --field-selector=status.phase=Running \
    -o jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' 2>/dev/null || echo "$TARGET")
  while IFS= read -r pod; do
    [[ -z "$pod" ]] && continue
    echo -n "    Copying to '$pod'... "
    kubectl cp "${NS_FLAG[@]}" "$LOCAL_JAR" "${pod}:/tmp/${PLUGIN_JAR_NAME}" 2>/dev/null && \
    ...
    echo -e "${GREEN}done${RESET}" || echo -e "${RED}FAILED${RESET}"
  done <<< "$DEPLOY_PODS"

  step "4" "Restart Dremio pods to load the plugin."
...
echo "  2. Go to Sources → Add Source → Neo4j"
echo "  3. Enter your Bolt URI (bolt://host:7687), username, and password"
echo "  4. Click Save — your Neo4j node labels will appear as tables"
echo ""
echo -e "${BOLD}Example queries:${RESET}"
echo "  SELECT * FROM neo4j_source.Person LIMIT 10;"
echo "  SELECT name, born FROM neo4j_source.Person WHERE born > 1970 ORDER BY born;"
echo "  SELECT title, released FROM neo4j_source.Movie ORDER BY released DESC LIMIT 5;"
echo ""
