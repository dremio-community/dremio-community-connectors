#!/usr/bin/env bash
# ============================================================================
# Dremio SingleStore Connector — Version-aware Rebuild Script
#
...
echo -e "${BOLD}${CYAN}   Dremio SingleStore Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
BUILD_FAILED=false
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c "rm -rf /tmp/singlestore-rebuild && mkdir -p /tmp/singlestore-rebuild"
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/singlestore-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/singlestore-rebuild/"
  docker exec -u root "$TARGET" bash -c "chmod -R 777 /tmp/singlestore-rebuild"
  if ! docker exec "$TARGET" bash -c "
    cd /tmp/singlestore-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/singlestore-rebuild && mkdir -p /tmp/singlestore-rebuild"
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/singlestore-rebuild -xf -
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/singlestore-rebuild && mvn package -DskipTests -q --batch-mode"; then
    BUILD_FAILED=true
  fi
  if ! $BUILD_FAILED; then
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/singlestore-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c \
    "cp /tmp/singlestore-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/ && \
     chown dremio:dremio /opt/dremio/jars/3rdparty/${PLUGIN_JAR_NAME}"
  docker cp "${TARGET}:/tmp/singlestore-rebuild/target/${PLUGIN_JAR_NAME}" \
