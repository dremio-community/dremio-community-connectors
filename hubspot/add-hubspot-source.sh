#!/usr/bin/env bash
# add-hubspot-source.sh — Add a HubSpot (CRM) source to Dremio via REST API
#
# PURPOSE
#   Creates or updates a HubSpot source in Dremio without going through the UI.
#   Authenticates to Dremio, then calls PUT /apiv2/source/{name} with your
#   HubSpot Private App token and configuration.
#
# USAGE
#   ./add-hubspot-source.sh --name <source_name> --token <pat-na1-...> [options]
#
# REQUIRED
#   --name          NAME      Dremio source name to create (e.g. "hubspot")
#   --token         TOKEN     HubSpot Private App access token (prompted if omitted)
#
# OPTIONAL
...
#   --page-size     N         Records per HubSpot API call, max 100 (default: 100)
#   --timeout       SECONDS   HTTP timeout for HubSpot calls (default: 60)
#   --include-archived        Include archived (deleted) records in results
#   --force                   Delete and recreate if source already exists
#   --dry-run                 Print payload without submitting
#   -h, --help                Show this help
#
# EXAMPLES
#   # Minimal — will prompt for HubSpot token and Dremio password
#   ./add-hubspot-source.sh --name hubspot
#
#   # Non-interactive (CI/CD)
#   ./add-hubspot-source.sh \
#     --name hubspot \
#     --token pat-na1-xxxxxxxxxxxx \
#     --user dremio_admin --password dremio_pass
#
#   # Replace existing source
#   ./add-hubspot-source.sh --name hubspot --token pat-na1-xxx --force
#
...
#                    crm.objects.owners.read (and any other object types you need)
#
# EXIT CODES
...
if [[ -z "$HS_TOKEN" ]]; then
  read -rsp "HubSpot Private App token (pat-na1-...): " HS_TOKEN </dev/tty; echo >&2
fi
...
config = {
    ...
    'includeArchived':      $([[ '$INCLUDE_ARCHIVED' == 'true' ]] && echo 'True' || echo 'False'),
}
...
HTTP=$(curl -s -o /tmp/_hs_source_resp.json -w "%{http_code}" \
...
if [[ "$HTTP" != "200" ]]; then
  ERR_MSG=$(python3 -c \
    "import sys,json; print(json.load(open('/tmp/_hs_source_resp.json')).get('errorMessage','unknown'))" \
    2>/dev/null || cat /tmp/_hs_source_resp.json)
  err "Source creation failed (HTTP $HTTP): $ERR_MSG"
fi

RESPONSE=$(cat /tmp/_hs_source_resp.json)
...
info "Available tables: contacts, companies, deals, tickets, products,"
info "                  line_items, calls, emails, meetings, notes, tasks, owners"
echo ""
echo "  Test with:"
echo "    SELECT id, firstname, lastname, email FROM \"$SOURCE_NAME\".contacts LIMIT 10;"
echo "    SELECT id, dealname, amount, dealstage FROM \"$SOURCE_NAME\".deals LIMIT 10;"
echo "    SELECT * FROM \"$SOURCE_NAME\".owners;"
echo ""
