#!/usr/bin/env bash
# =============================================================================
# Build script for Dremio ClickHouse Connector Kubernetes images
#
# Builds two Docker images:
#   1. Full custom Dremio image with ClickHouse plugin baked in (recommended)
#   2. Minimal init-container image that holds both plugin JARs
#
# Usage:
#   ./build.sh [OPTIONS]
#
...
if [[ -n "$REGISTRY" ]]; then
  FULL_IMAGE="${REGISTRY}/dremio-with-clickhouse:${DREMIO_TAG}"
  INIT_IMAGE="${REGISTRY}/dremio-clickhouse-plugin-init:${PLUGIN_VERSION}"
else
  FULL_IMAGE="dremio-with-clickhouse:${DREMIO_TAG}"
  INIT_IMAGE="dremio-clickhouse-plugin-init:${PLUGIN_VERSION}"
fi

echo "=== Dremio ClickHouse Connector — Kubernetes Image Builder ==="
...
# ---------------------------------------------------------------------------
# Build init container image (requires pre-built JARs)
# ---------------------------------------------------------------------------
if [[ "$BUILD_INIT" == true ]]; then
  echo ""
  echo "--- Building init container image ---"

  if ! ls "${SCRIPT_DIR}/target/dremio-clickhouse-connector-"*"-plugin.jar" &>/dev/null; then
    echo ""
  ...
  # Ensure the ClickHouse JDBC driver JAR is in target/ for Dockerfile.init
  JDBC_JAR="${SCRIPT_DIR}/target/clickhouse-jdbc-0.4.6-all.jar"
  if [[ ! -f "$JDBC_JAR" ]]; then
    echo "Copying ClickHouse JDBC driver to target/..."
    cp ~/.m2/repository/com/clickhouse/clickhouse-jdbc/0.4.6/clickhouse-jdbc-0.4.6-all.jar \
       "${SCRIPT_DIR}/target/" || {
      echo "ERROR: clickhouse-jdbc-0.4.6-all.jar not found in local Maven repo." >&2
      echo "       Run 'mvn package -DskipTests' first to download it." >&2
      exit 1
    }
  fi
