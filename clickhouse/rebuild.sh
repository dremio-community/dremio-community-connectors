#!/usr/bin/env bash
# ============================================================================
# Dremio ClickHouse Connector — Version-aware Rebuild Script
#
# PURPOSE
...
#   The ClickHouse JDBC driver (clickhouse-jdbc-0.4.6-all.jar) is version-stable
#   and does not need to change when Dremio upgrades — only the plugin JAR changes.
#
# USAGE
...
#
# WHAT IT DOES
#   1. Detects Dremio, Arrow, and Calcite JAR versions from the running instance
...
#      compiles the connector, deploys both the plugin JAR and driver JAR, restarts Dremio
#
# TWO-JAR DEPLOYMENT
#   ClickHouse requires two JARs in /opt/dremio/jars/3rdparty/:
#     • dremio-clickhouse-connector-1.0.0-SNAPSHOT-plugin.jar  (rebuilt per Dremio version)
#     • clickhouse-jdbc-0.4.6-all.jar                          (stable, version never changes)
#   Both are deployed to every pod on each rebuild.
#
# EXIT CODES
...
DRIVER_JAR_PATH="${SCRIPT_DIR}/jars/${DRIVER_JAR_NAME}"
...
echo -e "${BOLD}${CYAN}   Dremio ClickHouse Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"

# ── Ensure driver JAR is available locally ────────────────────────────────────
if [[ ! -f "$DRIVER_JAR_PATH" ]]; then
  echo ""
  warn "ClickHouse JDBC driver not found at: jars/${DRIVER_JAR_NAME}"
  info "Downloading from Maven Central..."
  mkdir -p "${SCRIPT_DIR}/jars"
  if command -v curl &>/dev/null; then
    curl -fsSL -o "$DRIVER_JAR_PATH" "$DRIVER_MAVEN_URL" && \
      ok "Downloaded ${DRIVER_JAR_NAME}" || \
      die "Download failed. Get it manually from: ${DRIVER_MAVEN_URL}"
  elif command -v wget &>/dev/null; then
    wget -q -O "$DRIVER_JAR_PATH" "$DRIVER_MAVEN_URL" && \
      ok "Downloaded ${DRIVER_JAR_NAME}" || \
      die "Download failed. Get it manually from: ${DRIVER_MAVEN_URL}"
  else
    die "curl/wget not found. Download manually:\n  ${DRIVER_MAVEN_URL}\n  → jars/${DRIVER_JAR_NAME}"
  fi
else
  ok "Driver JAR present: jars/${DRIVER_JAR_NAME}"
fi
...
if [[ -n "$CALCITE_JAR" ]]; then
  ...
  DETECTED_CALCITE=""
  warn "Calcite JAR not found in 3rdparty/ — will skip Calcite version update"
fi
...
CURRENT_CALCITE=$(sed -n 's|.*<calcite.version>\([^<]*\)</calcite.version>.*|\1|p' "$POM_XML" | head -1 || echo "")

info "pom.xml dremio.version  : ${CURRENT_DREMIO}"
[[ -n "$CURRENT_ARROW"   ]] && info "pom.xml arrow.version   : ${CURRENT_ARROW}"
[[ -n "$CURRENT_CALCITE" ]] && info "pom.xml calcite.version : ${CURRENT_CALCITE}"
...
[[ -n "$DETECTED_CALCITE" && -n "$CURRENT_CALCITE" && "$DETECTED_CALCITE" != "$CURRENT_CALCITE" ]] && VERSIONS_MATCH=false
...
if ! $VERSIONS_MATCH; then
  ...
  [[ -n "$DETECTED_CALCITE" && -n "$CURRENT_CALCITE" && "$DETECTED_CALCITE" != "$CURRENT_CALCITE" ]] && \
    warn "  calcite : ${CURRENT_CALCITE} → ${DETECTED_CALCITE}"
fi
...
if [[ -n "$DETECTED_CALCITE" && -n "$CURRENT_CALCITE" ]]; then
...
install_jar_from_target "${JARS}/dremio-services-credentials-${DV}.jar" com.dremio.services dremio-services-credentials "$DV"
install_jar_from_target "${JARS}/dremio-services-datastore-${DV}.jar" com.dremio          dremio-services-datastore   "$DV"
install_jar_from_target "${JARS}/dremio-services-options-${DV}.jar"   com.dremio          dremio-services-options     "$DV"

# CE JDBC plugin JARs (required by ClickHouse connector; found in main jars/ dir)
install_jar_from_target "${JARS}/dremio-ce-jdbc-plugin-${DV}.jar"     com.dremio.plugins  dremio-ce-jdbc-plugin       "$DV"
install_jar_from_target "${JARS}/dremio-ce-jdbc-fetcher-api-${DV}.jar" com.dremio.plugins dremio-ce-jdbc-fetcher-api  "$DV"
...
# Calcite JARs (used by JDBC planning rules)
if [[ -n "$DETECTED_CALCITE" ]]; then
  install_jar_from_target "${TP}/calcite-core-${DETECTED_CALCITE}.jar"   org.apache.calcite calcite-core   "$DETECTED_CALCITE"
  install_jar_from_target "${TP}/calcite-linq4j-${DETECTED_CALCITE}.jar" org.apache.calcite calcite-linq4j "$DETECTED_CALCITE"
fi
...
# For Docker build: copy the driver JAR into the container's Maven local repo
# so the build can include it as a dependency.
if [[ "$MODE" == "docker" ]]; then
  docker cp "$DRIVER_JAR_PATH" "${TARGET}:/tmp/${DRIVER_JAR_NAME}"
  docker exec "$TARGET" bash -c "
    mvn install:install-file -q \
      -Dfile=/tmp/${DRIVER_JAR_NAME} \
      -DgroupId=com.clickhouse \
      -DartifactId=clickhouse-jdbc \
      -Dversion=0.4.6 \
      -Dclassifier=all \
      -Dpackaging=jar 2>/dev/null
  " && ok "Driver JAR installed in container Maven repo" || \
     warn "Driver JAR install skipped (may already be present)"
fi

BUILD_FAILED=false

if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c "rm -rf /tmp/clickhouse-rebuild && mkdir -p /tmp/clickhouse-rebuild"
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/clickhouse-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/clickhouse-rebuild/"
  docker exec -u root "$TARGET" bash -c "chmod -R 777 /tmp/clickhouse-rebuild"

  if ! docker exec "$TARGET" bash -c "
    cd /tmp/clickhouse-rebuild
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/clickhouse-rebuild && mkdir -p /tmp/clickhouse-rebuild"
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/clickhouse-rebuild -xf -
  # Copy driver JAR into pod and install into Maven local repo
  kubectl cp "${NS_FLAG[@]}" "$DRIVER_JAR_PATH" "${TARGET}:/tmp/${DRIVER_JAR_NAME}"
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c "
    mvn install:install-file -q \
      -Dfile=/tmp/${DRIVER_JAR_NAME} \
      -DgroupId=com.clickhouse \
      ...
      -Dclassifier=all \
      -Dpackaging=jar 2>/dev/null
  " && ok "Driver JAR installed in pod Maven repo" || true
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/clickhouse-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  if ! $BUILD_FAILED; then
    mkdir -p "${SCRIPT_DIR}/jars"
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/clickhouse-rebuild/target/${PLUGIN_JAR_NAME}" \
...
if $BUILD_FAILED; then
  ...
  echo    "    • A JDBC planning rule base class changed (RelOptRule / JdbcRel errors)"
  echo    "    • CE JDBC plugin API changed (JdbcStoragePlugin method signatures)"
  ...
  echo    "    2. Fix the affected Java file(s)"
...
# ── Step 6: Deploy plugin JAR + driver JAR ────────────────────────────────────
step "6" "Deploying plugin JAR and driver JAR..."

if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" bash -c "
    cp /tmp/clickhouse-rebuild/target/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/ && \
    cp /tmp/${DRIVER_JAR_NAME} /opt/dremio/jars/3rdparty/
  "
  docker cp "${TARGET}:/tmp/clickhouse-rebuild/target/${PLUGIN_JAR_NAME}" \
  ...
  cp "$DRIVER_JAR_PATH" "${TARGET}/jars/3rdparty/"
  ...
  info "Deploying both JARs to ${POD_COUNT} pod(s)${SELECTOR:+ (selector: $SELECTOR)}..."
  ...
  while IFS= read -r pod; do
    ...
    kubectl cp "${NS_FLAG[@]}" \
      "$DRIVER_JAR_PATH" "${pod}:/tmp/${DRIVER_JAR_NAME}"
    kubectl exec "${NS_FLAG[@]}" "$pod" -- bash -c "
      cp /tmp/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/ && \
      cp /tmp/${DRIVER_JAR_NAME} /opt/dremio/jars/3rdparty/ && \
      chown dremio:dremio \
        /opt/dremio/jars/3rdparty/${PLUGIN_JAR_NAME} \
        /opt/dremio/jars/3rdparty/${DRIVER_JAR_NAME} 2>/dev/null || true"
    ok "Deployed both JARs to ${pod}"
  done <<< "$DEPLOY_PODS"
fi

ok "JARs deployed to 3rdparty/"
...
echo -e "  Driver JAR     : ${DRIVER_JAR_NAME} (unchanged)"
