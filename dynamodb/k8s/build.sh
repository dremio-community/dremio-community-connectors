#!/usr/bin/env bash
# =============================================================================
# Build script for Dremio DynamoDB Connector Kubernetes images
#
# Builds two Docker images:
#   1. Full custom Dremio image with DynamoDB plugin baked in (recommended)
...
if [[ -n "$REGISTRY" ]]; then
  FULL_IMAGE="${REGISTRY}/dremio-with-dynamodb:${DREMIO_TAG}"
  INIT_IMAGE="${REGISTRY}/dremio-dynamodb-plugin-init:${PLUGIN_VERSION}"
else
  FULL_IMAGE="dremio-with-dynamodb:${DREMIO_TAG}"
  INIT_IMAGE="dremio-dynamodb-plugin-init:${PLUGIN_VERSION}"
fi

echo "=== Dremio DynamoDB Connector — Kubernetes Image Builder ==="
...
if [[ "$BUILD_INIT" == true ]]; then
  ...
  if ! ls "${SCRIPT_DIR}/target/dremio-dynamodb-connector-"*"-plugin.jar" &>/dev/null; then
    echo ""
