#!/usr/bin/env bash
# ============================================================================
# Dremio Zendesk Connector — Version-aware Rebuild Script
#
# PURPOSE
...
echo -e "${BOLD}${CYAN}   Dremio Zendesk Connector — Version-aware Rebuild${RESET}"
echo -e "${BOLD}${CYAN}════════════════════════════════════════════════════════════${RESET}"
...
if [[ "$MODE" == "docker" ]]; then
  docker exec "$TARGET" mkdir -p /tmp/zendesk-rebuild
  docker cp "${SCRIPT_DIR}/src"     "${TARGET}:/tmp/zendesk-rebuild/"
  docker cp "${SCRIPT_DIR}/pom.xml" "${TARGET}:/tmp/zendesk-rebuild/"

  if ! docker exec "$TARGET" bash -c "
    cd /tmp/zendesk-rebuild
  ...
  kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "rm -rf /tmp/zendesk-rebuild && mkdir -p /tmp/zendesk-rebuild"
  # Copy source into the pod
  tar -C "$SCRIPT_DIR" --exclude='./target' --exclude='./jars' -cf - . | \
    kubectl exec "${NS_FLAG[@]}" -i "$TARGET" -- tar -C /tmp/zendesk-rebuild -xf -
  # Build
  if ! kubectl exec "${NS_FLAG[@]}" "$TARGET" -- bash -c \
    "cd /tmp/zendesk-rebuild && mvn package -DskipTests -q --batch-mode 2>&1"; then
    BUILD_FAILED=true
  fi
  ...
  if ! $BUILD_FAILED; then
    ...
    kubectl cp "${NS_FLAG[@]}" \
      "${TARGET}:/tmp/zendesk-rebuild/jars/${PLUGIN_JAR_NAME}" \
...
if [[ "$MODE" == "docker" ]]; then
  docker exec -u root "$TARGET" bash -c \
    "cp /tmp/zendesk-rebuild/jars/${PLUGIN_JAR_NAME} /opt/dremio/jars/3rdparty/"
  # Pull the freshly-built JAR back to host jars/ so it can be used as prebuilt next time
  docker cp "${TARGET}:/tmp/zendesk-rebuild/jars/${PLUGIN_JAR_NAME}" \
