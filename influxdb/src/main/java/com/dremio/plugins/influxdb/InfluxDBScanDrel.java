...
public class InfluxDBScanDrel extends ScanRelBase implements Rel {

    private final InfluxDBScanSpec scanSpec;

    public InfluxDBScanDrel(RelOptCluster cluster, RelTraitSet traitSet, RelOptTable table,
            StoragePluginId pluginId, TableMetadata tableMetadata,
            List<SchemaPath> projectedColumns, double observedRowcountAdjustment,
            List<RelHint> hints, InfluxDBScanSpec scanSpec) {
        super(cluster, traitSet, table, pluginId, tableMetadata,
                projectedColumns, observedRowcountAdjustment, hints);
        this.scanSpec = scanSpec;
    }

    public InfluxDBScanSpec getScanSpec() { return scanSpec; }

    @Override
    public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
        return new InfluxDBScanDrel(getCluster(), traitSet, getTable(), getPluginId(),
    ...
    public ScanRelBase cloneWithProject(List<SchemaPath> columns) {
        return new InfluxDBScanDrel(getCluster(), getTraitSet(), getTable(), getPluginId(),
